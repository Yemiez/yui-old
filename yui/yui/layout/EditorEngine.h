...
public:
    ...
    [[nodiscard]] auto column() const { return m_column; }
    ...
    bool operator<(const TextPosition &rhs) const {
        return m_line < rhs.m_line || (m_line == rhs.m_line && m_column < rhs.m_column);
    }
...
public:
    ...
    [[nodiscard]] const TextPosition &start() const { return m_start; }
    [[nodiscard]] const TextPosition &end() const { return m_end; }
    TextPosition &end() { return m_end; }
    void set_start(TextPosition pos) { m_start = pos; }
    void set_end(TextPosition pos) { m_end = pos; }
    void set(TextPosition start, TextPosition end) {
        m_start = start;
        m_end = end;
    }
    ...
    [[nodiscard]] bool contains(const TextPosition &position) const { return position >= m_start && position <= m_end; }
    [[nodiscard]] bool is_same_line() const { return m_start.line() == m_end.line(); }
private:
...
public:
    explicit TextDocumentLine(TextDocument *document);
    TextDocumentLine(TextDocument *document, Utf8String);

    [[nodiscard]] const Utf8String &text() const { return m_text; }
    Utf8String &text() { return m_text; }
    [[nodiscard]] uint32_t length() const { return m_text.length(); }
...
public:
    explicit TextDocument(EditorEngine *editor);

    // Document client.
    void connect_client(Client *client) { m_client = client; }
    void disconnect_client() { m_client = nullptr; }

    [[nodiscard]] const auto *client() const { return m_client; }
    auto *client() { return m_client; }

    [[nodiscard]] uint32_t line_count() const { return m_lines.size(); }

    [[nodiscard]] const auto &line(uint32_t idx) const { return m_lines.at(idx); }
    auto &line(uint32_t idx) { return m_lines.at(idx); }
    [[nodiscard]] const auto &lines() const { return m_lines; }
...
public: // Text functions
    ...
    [[nodiscard]] Utf8String text() const;
    [[nodiscard]] Utf8String text_in_range(TextRange) const;

    [[nodiscard]] uint32_t code_point_at(const TextPosition &) const;

    [[nodiscard]] TextRange range_for_entire_document() const;
    [[nodiscard]] TextRange range_for_entire_line(uint32_t index) const;
    [[nodiscard]] TextPosition start_of_line(uint32_t index) const;
    [[nodiscard]] TextPosition end_of_line(uint32_t index) const;
    [[nodiscard]] TextPosition next_position_after(const TextPosition &) const;
    [[nodiscard]] TextPosition previous_position_after(const TextPosition &) const;
    [[nodiscard]] TextPosition next_position_up(const TextPosition &) const;
    [[nodiscard]] TextPosition next_position_down(const TextPosition &) const;
    [[nodiscard]] TextPosition next_word_break_after(const TextPosition &) const;
    [[nodiscard]] TextPosition previous_word_break_after(const TextPosition &) const;

    [[nodiscard]] bool contains(const TextPosition &) const;
...
public:
    ...
    explicit EditorEngine(EditorEngineType type);

    [[nodiscard]] bool is_single_line() const { return m_type == EditorEngineType::SingleLine; }
    [[nodiscard]] bool is_multi_line() const { return m_type == EditorEngineType::MultiLine; }
    [[nodiscard]] const TextDocument &text_document() const { return m_document; }
    ...
    [[nodiscard]] bool has_selection() const { return m_selection.start() != m_selection.end(); }
protected:
