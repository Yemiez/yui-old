...
void yui::layout::DocumentWidget::update(float dt)
{
	...
	if (m_dirty_layout) {
		...
		switch (new_cursor) {
		case ComputedCursorMode::Input: m_window->use_input_cursor(); break;
		case ComputedCursorMode::Hand: m_window->use_hand_cursor(); break;
...
{
	if ((m_construction_stack.top()->is_inline() && !m_construction_stack.top()->is_inline_box()) && dom_node->is_fragment()) {
...
{
	return std::find_if(m_loaded_fonts.begin(), m_loaded_fonts.end(), [&](auto *font) {
...
{
	const auto it = std::find_if(m_loaded_fonts.begin(), m_loaded_fonts.end(), [&](auto *font) {
...
const yui::layout::LayoutNode* yui::layout::DocumentWidget::find_layout_node(Node* dom_node) const
{
	...
	traverse([&](const LayoutNode* node) {
...
void yui::layout::DocumentWidget::load_fonts()
{
	...
	traverse([&](LayoutNode *node) {
		...
		if (font == nullptr) {
			Application::the().report_error(yui::fmt("Could not load font %s (with size=%d)", text.font_name, text.font_size));
			return;
		}
