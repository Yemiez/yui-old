...
void yui::layout::BoxCompute::compute_rows(LayoutNode& box)
{
	...
	for (auto i = 0u, sz = box.children().size(); i < sz; ++i) {
		...
		if (prev && prev->is_inline() && !child->is_inline()) {
			inline_ranges.emplace_back(std::pair<uint32_t, uint32_t>{first_inline_idx, i});
		...
		auto x_accum = child->dom_node()->computed().margin().left + child->size_with_padding().x + child->dom_node()->computed().margin().right;
		if (x_accum > size.x) {
			size.x = x_accum;
		}
	}
	...
	if (first_inline_idx != static_cast<uint32_t>(-1)) {
		inline_ranges.emplace_back(std::pair<uint32_t, uint32_t>{first_inline_idx, box.children().size()});
	}
