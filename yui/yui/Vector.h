...
public:
    ...
    [[nodiscard]] bool is_null() const { return m_buffer == nullptr; }
...
void Vector<T>::resize(u32 new_size, const T &value) {
    if (new_size < m_size) {
        return shrink_size(new_size); // Possibly an error because it passed a default value which will be ignored.
