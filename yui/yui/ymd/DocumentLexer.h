...
#define DOCUMENT_TOKEN_ENUMERATOR \
	_DOCUMENT_TOKEN_ENUMERATOR_(None, none) \
	_DOCUMENT_TOKEN_ENUMERATOR_(Eof, eof) \
	_DOCUMENT_TOKEN_ENUMERATOR_(Character, character) \
	_DOCUMENT_TOKEN_ENUMERATOR_(OpenTagOpen, open_tag_open) \
	_DOCUMENT_TOKEN_ENUMERATOR_(OpenTagName, open_tag_name) \
	_DOCUMENT_TOKEN_ENUMERATOR_(OpenTagClose, open_tag_close) \
	_DOCUMENT_TOKEN_ENUMERATOR_(OpenTagSelfClosing, open_tag_self_closing) \
	_DOCUMENT_TOKEN_ENUMERATOR_(OpenTagAttributeName, open_tag_attribute_name) \
	_DOCUMENT_TOKEN_ENUMERATOR_(OpenTagAttributeEqual, open_tag_attribute_equal) \
	_DOCUMENT_TOKEN_ENUMERATOR_(OpenTagAttributeQuoteStart, open_tag_attribute_quote_start) \
	_DOCUMENT_TOKEN_ENUMERATOR_(OpenTagAttributeValueFragment, open_tag_attribute_value_fragment) \
	_DOCUMENT_TOKEN_ENUMERATOR_(OpenTagAttributeQuoteEnd, open_tag_attribute_quote_end) \
	_DOCUMENT_TOKEN_ENUMERATOR_(ClosingTagOpen, closing_tag_open) \
	_DOCUMENT_TOKEN_ENUMERATOR_(ClosingTagName, closing_tag_name) \
	_DOCUMENT_TOKEN_ENUMERATOR_(ClosingTagClose, closing_tag_close) 
	
	
	enum class DocumentTokenType
	{
#define _DOCUMENT_TOKEN_ENUMERATOR_(x, y) x,
		DOCUMENT_TOKEN_ENUMERATOR
#undef _DOCUMENT_TOKEN_ENUMERATOR_
	};
	...
	public:
		...
		StreamPosition end_position() const { return m_start_position; }
		const std::string& content() const { return m_content; }
		DocumentTokenType type() const { return m_type; }

		// is_x functions
#define _DOCUMENT_TOKEN_ENUMERATOR_(x, y) bool is_##y() const { return m_type == DocumentTokenType::x; }
		DOCUMENT_TOKEN_ENUMERATOR
#undef _DOCUMENT_TOKEN_ENUMERATOR_
	...
	public:
		...
		DocumentLexer(std::string);

		bool reached_eof() const;
		DocumentToken next();
		DocumentToken peek() const;

		bool has_error() const { return !m_errors.empty(); }
		const std::vector<DocumentLexError>& errors() const { return m_errors; }
		const std::vector<DocumentToken>& tokens() const { return m_tokens; }
