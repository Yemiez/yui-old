...
{
	return traverse(this, [&selector](Node* node) -> Node* {
...
{
	auto it = std::find_if(m_stylesheets.begin(), m_stylesheets.end(), [&file](const Stylesheet &sheet) {
...
{
	const auto iterator = std::find_if(m_id_map.begin(), m_id_map.end(), [&](const auto& pair) {
