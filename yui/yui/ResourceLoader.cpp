...
yui::FontResource::~FontResource() {
    ...
    spdlog::info("Deleted font face ('{}', {}px)", m_font_path, m_pixel_size);
}
...
yui::FontResource *yui::ResourceLoader::load_font(std::string path, uint32_t pixel_size) {
    ...
    spdlog::info("Loaded font '{}' {}px", font->path(), font->font_size());
    return font;
}
...
yui::Shader *yui::ResourceLoader::load_shader(const std::string &path) {
    try {
        ...
        spdlog::info("Loaded vertex & fragment shader '{}'", path);
        return shader;
    }
    catch (std::exception &) {
        spdlog::error("Could not load vertex & fragment shader '{}'", path);
        return nullptr;
    }
}
