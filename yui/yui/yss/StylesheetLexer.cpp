...
yui::StylesheetToken::StylesheetToken(
...
const char *yui::StylesheetToken::type_to_string(StylesheetTokenType type) {
    switch (type) {
#define STYLESHEET_TOKEN_ENUMERATOR_(x, ...) case StylesheetTokenType::x: return #x;
...
yui::StylesheetLexer::LexState yui::StylesheetLexer::lex_expecting_semicolon() {
    ...
    emit_error(
            m_reader.position(),
            yui::fmt("Unknown character when expecting semicolon or closing brace: %d %c", static_cast<int>(ch), ch)
    );
    return LexState::Error;
}
...
void yui::StylesheetLexer::emit_token(
        StreamPosition start,
        StreamPosition end,
        std::string content,
        StylesheetTokenType type
) {
    m_tokens.emplace_back(start, end, std::move(content), type);
}
...
void yui::StylesheetLexer::emit_error(StreamPosition pos, std::string msg) {
    m_errors.emplace_back(pos, std::move(msg));
}
