...
yui::ImmutableComputedValues yui::ComputedValues::immutable_from_style(const StylesheetDeclaration& style, Node& dom_node)
{
...
{
	if (m_inherited.text.color == Color{0, 0, 0, 0}) {
		m_inherited.text.color = other.text().color;
	}
	if (m_inherited.text.font_name.empty()) {
		m_inherited.text.font_name = other.text().font_name;
	}
	if (m_inherited.text.font_size == 0) {
