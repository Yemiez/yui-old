...
{
	switch (relation) {
		case Relation::ImmediateChild: return ">";
		case Relation::Descendant: return "";
		case Relation::AdjacentSibling: return "+";
	}
...
uint32_t yui::Selector::weight() const
{
	...
	for (const auto& simple : m_complex_selectors) {
		for (const auto &part : simple.parts()) {
			switch (part.type) {
				case SimpleSelector::Type::Universal: ++weight; break;
				case SimpleSelector::Type::TagName: weight += 10; break;
				case SimpleSelector::Type::Id: weight += 100; break;
				case SimpleSelector::Type::Class: weight += 10; break;
			}
		}
...
bool yui::Selector::match(Node& node, uint32_t selector_index) const
{
	...
	for (const auto &part : selector.parts()) {
		switch (part.type) {
			case SimpleSelector::Type::Invalid: return false;
			case SimpleSelector::Type::Universal: break;
			case SimpleSelector::Type::TagName: 			
	...
	case SimpleSelector::PseudoClass::None: break;
	case SimpleSelector::PseudoClass::Hover: 
		if (!node.hovered()) {
			return false;
		}
