...
yui::Utf8String yui::Clipboard::content() const
{
	...
	log::debug("Retrieved clipboard: {}", content.to_printable_string());
	return content;
}

void yui::Clipboard::set_content(const Utf8String& content)
{
	log::debug("Setting clipboard content to: {}", content.to_printable_string());
...
{
	log::debug("Setting clipboard content to: {}", content);
